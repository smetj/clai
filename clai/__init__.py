...
import importlib
import sys
from clai.tools import (get_backend_instance_config, get_exit_code,
...
def main():
    try:
        ...
        backend_config = get_backend_instance_config(
            config=read_config(args.config), backend=args.backend, instance=args.instance
        )

        Client = getattr(importlib.import_module(f"clai.backend.{args.backend}"), "Client")
